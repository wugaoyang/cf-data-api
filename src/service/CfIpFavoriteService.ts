...
export default class CfIpFavoriteService {
	/**
	 * 分页查询
	 * @param request
	 * @param env
	...
	static async page(request: Request, env: Env) {
		...
		let countSql = 'SELECT count(1) total FROM cf_ip_favorite where 1=1 ' + condition;
		// console.log(countSql);
		let result = await env.DB.prepare(countSql).all();
		let total = result.results[0].total;
		let querySql = 'SELECT * FROM cf_ip_favorite where 1=1 ' + condition + ' order by updatedTime desc limit ?,?';
	...
	static async getAllReachable(env: Env) {
		const { results } = await env.DB.prepare(
			'SELECT * FROM cf_ip_favorite WHERE status in(0) and delay > 0'
		).all();
	...
	static async deleteDisableIp(env: Env) {
		await env.DB.exec(
			'delete FROM cf_ip_favorite WHERE status in(0) or delay <= 0 or speed < 10'
		);
	...
	static async list(request: Request, env: Env) {
		...
		const { results } = await env.DB.prepare(
			'SELECT * FROM cf_ip_favorite order by updatedTime desc ' + limitCondition
		).all();
		return Result.succeed(JSON.stringify(results));
	}
	...
	static async doAdd(IpInfos: IpInfo[], env: Env) {
		...
		let insertSql = 'INSERT INTO cf_ip_favorite (ip,' +
			'cityNameCN,' +
			'cityNameEN,' +
			'continentCode,' +
			'countryCode,' +
	...
	static async update(request: Request, env: Env) {
		try {
			...
			if (deleteOld && deleteOld === '1') {
				await env.DB.exec(
					'DELETE FROM cf_ip_favorite WHERE  `group` =\'' + group + '\''
				);
			}
			let bestIps = '';
			// @ts-ignore
			...
			if (bestIps) {
				...
				let insertSql = 'INSERT INTO cf_ip_favorite (ip, name,`group`,  speed , status, `source`, updatedTime) VALUES';
				let countryCodeMap: Map<string, string> = await CommonUtil.getCountryCodeBatch(ips);
